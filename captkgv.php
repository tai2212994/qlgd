...
if(isset($_POST['btnregister']))
{
		...
		$sql="INSERT INTO taikhoan(TK_MA,MATKHAU,quyen) 			values('$u','$p',1)";
			...
			if($result)
                ...
                <script language="javascript">
					...
					window.location="index.php?top=40";
                </script>
                <?php
			}
		else
  ...
  <tr>
    ...
    <td width="530" height="50"><select name='TK_MA'>
			             <?php
                          ...
                          include('dbconect.php');
			                     $qry="select * from giaovien gv left join taikhoan tk on gv.gv_ma = tk.TK_ma where tk.TK_MA is null";
			                     $result = mysql_query($qry);
			                     while ($row = mysql_fetch_array($result))
			                    {
                                    echo "<option value='".$row["GV_MA"]."'>".$row["GV_MA"];
                                }
			             ?>
                        </select></td>
  </tr>
...
<td> <input id= 'txtpass' name = 'txtpass' value="">
 </td>
  </tr>
  <tr>
